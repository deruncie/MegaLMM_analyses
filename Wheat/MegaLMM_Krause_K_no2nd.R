...
runID = sprintf('MegaLMM_Krause_no2nd_K_%d',foldid)
...
# predict_MegaLMM = function(data,HTP_wide,K_year,runID,nas) {
  ...
  results = data.frame(Method = 'MegaLMM_K_0',
  ...
  results$fold = foldid
  write.csv(results,file = sprintf('%s/results_MegaLMM_no2nd_fold_%d.csv',results_dir,foldid))
