...
# runID = sprintf('/group/runciegrp/Projects/MegaLMM/Krause/MegaLMM_Krause_RKHS_%d',foldid)
runID = sprintf('MegaLMM_Krause_RKHS_%d',foldid)
...
data$GID1 = data$GID2 = data$GID3 = data$GID
...
run_parameters = MegaLMM_control(
  ...
  h2_divisions = 10, # Each variance component is allowed to explain between 0% and 100% of the total variation. How many segments should the range [0,100) be divided into for each random effect?
  h2_step_size = .3, # if NULL, all possible values of random effects are tried each iteration. If in (0,1), a new candidate set of random effect proportional variances is drawn uniformily with a range of this size
...
MegaLMM_state = setup_model_MegaLMM(Y,            # n x p data matrix
                              ~ 1 + (1|GID1) + (1|GID2) + (1|GID3),
                              data=data,         # the data.frame with information for constructing the model matrices
                              relmat = list(GID1 = K1, GID2 = K2, GID3 = K3), # covariance matrices for the random effects. If not provided, assume uncorrelated
...
for(i  in 1:200) {
  ...
  U = get_posterior_FUN(MegaLMM_state,Z %*% U_R[,1:2,drop=F] + Z %*% U_F %*% Lambda[,1:2,drop=F])
  U = U[,nas,1]
  ...
  if(MegaLMM_state$current_state$nrun < MegaLMM_state$run_parameters$burn || i <= 100) {
...
U = get_posterior_mean(MegaLMM_state,Z %*% U_R[,1:2,drop=F] + Z %*% U_F %*% Lambda[,1:2,drop=F),bychunk = T)

results = data.frame(Method = 'MegaLMM_RKHS',
...
results$fold = foldid
write.csv(results,file = sprintf('%s/results_MegaLMM_RKHS_fold_%d.csv',results_dir,foldid))
